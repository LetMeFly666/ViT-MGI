...
class STEFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        upsample = nn.ConvTranspose2d(in_channels=1024,
                        out_channels=3,
                        kernel_size=16,
                        stride=16)
        upsample.weight = torch.nn.Parameter(((torch.rand((1024, 3, 16, 16))-0.5)/100))
        upsample.bias = torch.nn.Parameter(torch.zeros_like(upsample.bias))
        upsampled = upsample(grad_output.cpu(), output_size=(3,3,224,224)).to(device) # TODO change to 14 when needed?
        return upsampled
...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        self.patch_embeddings = Conv2d(in_channels=in_channels,
                                       ...
                                       stride=patch_size)
        global patch_embed
        patch_embed = self.patch_embeddings
    ...
    def forward(self, x):
        ...
        self.embeddings = x + self.position_embeddings
        self.embeddings = self.dropout(self.embeddings)
        return self.embeddings

class STEFunctionLayerNorm(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        ln = LayerNorm(1024, eps=1e-6)
        ln.weight = torch.nn.Parameter(checkptdict['transformer.encoder.layer.0.attention_norm.weight']).to(device)
        ln.bias = torch.nn.Parameter(checkptdict['transformer.encoder.layer.0.attention_norm.bias']).to(device)
        output = ln(input)
        return output
...
class StraightThroughEstimatorLayerNorm(nn.Module):
    def __init__(self):
        super(StraightThroughEstimator, self).__init__()
    def forward(self, x):
        x = STEFunctionLayerNorm.apply(x)
        return x
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids):
        ...
        self.encoded, attn_weights = self.encoder(embedding_output)
        #self.encoded.retain_grad()
        return self.encoded, attn_weights
...
class VisionTransformer(nn.Module):
    ...
    def load_from(self, weights):
        with torch.no_grad():
            if self.zero_head and os.getenv("DATASET") != "IMAGENET":
